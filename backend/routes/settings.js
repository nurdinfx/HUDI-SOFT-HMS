...
router.get('/', (req, res) => {
    const row = db.prepare('SELECT * FROM hospital_settings WHERE id = 1').get();
...
router.put('/', (req, res) => {
    ...
    const existing = db.prepare('SELECT * FROM hospital_settings WHERE id = 1').get();
    const { name, tagline, address, phone, email, website, currency, taxRate, logo } = req.body;
    if (!existing) {
        db.prepare('INSERT INTO hospital_settings (id, name, tagline, address, phone, email, website, currency, tax_rate, logo) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .run(name || 'Hospital', tagline || '', address || '', phone || '', email || '', website || '', currency || 'USD', taxRate || 10, logo || null);
    } else {
        db.prepare('UPDATE hospital_settings SET name=?, tagline=?, address=?, phone=?, email=?, website=?, currency=?, tax_rate=?, logo=? WHERE id=1')
    ...
    res.json(fmt(db.prepare('SELECT * FROM hospital_settings WHERE id = 1').get()));
});
