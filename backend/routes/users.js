...
router.post('/', (req, res) => {
    ...
    const id = uuidv4();
    db.prepare('INSERT INTO users (id, name, email, password_hash, role, phone, department, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)')
    ...
    res.status(201).json(fmt(db.prepare('SELECT * FROM users WHERE id = ?').get(id)));
});
...
router.put('/:id', (req, res) => {
    ...
    db.prepare('UPDATE users SET name=?, email=?, password_hash=?, role=?, phone=?, department=?, is_active=? WHERE id=?')
    ...
    res.json(fmt(db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id)));
});
